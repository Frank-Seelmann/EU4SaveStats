...
pub fn extract_current_state(
    ...
    let country = query.country(&country_tag.parse()?).ok_or("Country not found")?;
    ...
    for point in query.income_statistics_ledger(nation_events_ref).iter()
...
pub fn extract_historical_events(events: &[(eu4save::Eu4Date, eu4save::models::CountryEvent)]) -> Vec<HistoricalEvent> {
